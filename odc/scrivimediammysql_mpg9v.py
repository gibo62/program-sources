...
def insert_row(campi):
    if not campi[2][len(campi[2])-1].isdigit():
        ...
        campi[2]=campi[2][-4:]
    campi.insert(0,campi[2]+campi[3])
    sql="insert into media_mpg9v("
    if len(campi)> len (nometipo1):
        if campi.index("VIDEO_TS") == 9:
            campi[10]=campi[8]
            campi.pop(8)
...
nometipo1=["keyvideocoll","drive","tipo","num_video","collezione","versione","campo1","dir1","dir2","nomefile","data_modifica","data_accesso","data_creazione"]
